...
evalE (HiExprApply applyFun list) = evalE applyFun >>= \case
  HiValueFunction fun -> case fun of
    ...
    HiFunToUpper        -> one -|>|> (cons . T.toUpper)
    HiFunToLower        -> one -|>|> (cons . T.toLower)
    HiFunTrim           -> one -|>|> (cons . strip)
    HiFunReverse        -> funTextRev
    HiFunList           -> return -|>|> (cons . S.fromList)
    HiFunRange          ->
      (two @Rational) -|>|> \(a, b) -> cons . S.fromList $ fmap cons [a ..b]
    HiFunFold           -> oneone                               -|>|>* foldFun
    HiFunPackBytes      -> (one >=> return . (HiFunPackBytes,)) -|>|>* foldFun
    HiFunUnpackBytes    ->
      one -|>|> (cons . S.fromList . fmap (cons . toRational) . B.unpack)
    HiFunEncodeUtf8     -> one -|>|> (cons . encodeUtf8)
    HiFunDecodeUtf8     -> one -|>|> (either (\_ -> cons ()) cons . decodeUtf8')
    HiFunZip            -> zipFun
      (compressWith defaultCompressParams {compressLevel = bestCompression})
    HiFunUnzip          -> zipFun (decompressWith defaultDecompressParams)
    HiFunSerialise      -> one -|>|> (cons . L.toStrict . serialise @HiValue)
    HiFunDeserialis     ->
      one -|>|> (either (\_ -> cons ()) id . deserialiseOrFail . L.fromStrict)
    HiFunRead           -> one -|>|> (cons . HiActionRead  . T.unpack)
    HiFunWrite          -> evalActionWR
    HiFunMkDir          -> one -|>|> (cons . HiActionMkDir . T.unpack)
    HiFunChDir          -> one -|>|> (cons . HiActionChDir . T.unpack)
    HiFunParseTime      ->
      one -|>|> (maybe (cons ()) (cons @UTCTime) . readMaybe . T.unpack)
    HiFunRand           -> evalActionWR
    HiFunEcho           -> one -|>|> (cons . HiActionEcho)
 ...
 where
  ...
  funDivByZero = two             -|>|> (cons . uncurry slesh)
             <|> (two @Rational) -|>|>* div
    where
  ...
  funTwoArgMinus = two -|>|> (cons . uncurry ((-) @Rational))
               <|> two -|>|> (cons . uncurry fun)
    where
      fun t1 t2 = (toRational $ diffUTCTime t1 t2)

  funTwoArgAdd :: ExceptT HiError m HiValue
  funTwoArgAdd = two    -|>|> (cons . uncurry ((+) @Rational))
             <|> two    -|>|> (cons . uncurry T.append)
             <|> two    -|>|> (cons . uncurry ((><) @HiValue))
             <|> two    -|>|> (cons . uncurry B.append)
             <|> oneone -|>|> (cons . uncurry fun)
    where
      fun time (fromRational -> num) =
        addUTCTime (secondsToNominalDiffTime (num)) time

  funTwoArgMul :: ExceptT HiError m HiValue
  funTwoArgMul = two -|>|> (cons . uncurry ((*) @Rational))
             <|> (oneone @(Sem Text)          @Integer) -|>|> fun
             <|> (oneone @(Sem (Seq HiValue)) @Integer) -|>|> fun
             <|> (oneone @(Sem ByteString)    @Integer) -|>|> fun
    where
      fun ((Sem s), int) = cons $ (stimes int s)

  funTwoArgEq :: (HiValue -> HiValue -> Bool) -> ExceptT HiError m HiValue
  funTwoArgEq fun = return -|>|> (\[a, b] -> cons $ fun a b)

  funTextLen :: ExceptT HiError m HiValue
  funTextLen = one -|>|> (toNum . T.length)
           <|> one -|>|> (toNum . S.length @HiValue)
    where toNum = cons . toRational

  funTextRev :: ExceptT HiError m HiValue
  funTextRev = one -|>|> (cons . T.reverse)
           <|> one -|>|> (cons . S.reverse @HiValue)
  ...
  zipFun fun = one -|>|> (cons . L.toStrict . fun . L.fromStrict)

  evalActionWR :: ExceptT HiError m HiValue
  evalActionWR = two        -|>|> (cons . funWrite)
             <|> oneone     -|>|> (cons . funWrite')
             <|> (two @Int) -|>|> (cons . uncurry HiActionRand)
    where
  ...
  parseJSON fun = one -|>|> (cons . S.fromList . sort . fmap fun . M.toList)

  countString :: ExceptT HiError m HiValue
  countString = one -|>|> (toDict . fmap toString . T.unpack)
            <|> one -|>|> (toDict . F.toList @(Seq))
            <|> one -|>|> (toDict . fmap toByte . B.unpack)
    where
  ...
  invertJSON = one -|>|> (cons . M.fromListWith concat0 . pairs)
    where
